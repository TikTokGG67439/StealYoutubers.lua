...
toggleBox.Size = UDim2.new(0,60,0,20); toggleBox.Position = UDim2.new(0,16,0,60)
toggleBox.Text = "F"; toggleBox.Font = Enum.Font.Arial; toggleBox.TextScaled = true

local tpBox = Instance.new("TextBox", gui1)
tpBox.Size = UDim2.new(0,60,0,20); tpBox.Position = UDim2.new(0,80,0,60)
tpBox.Text = "T"; tpBox.Font = Enum.Font.Arial; tpBox.TextScaled = true
...
-- GUI1 visibility via FastSpeedSteal
buttonStates.FastSpeedSteal.Changed:Connect(function(val) gui1.Enabled = val end)
gui1.Enabled = buttonStates.FastSpeedSteal.Value

-- Input handling
UIS.InputBegan:Connect(function(input, processed)
    if processed then return end
    if input.KeyCode == toggleKey then buttonStates.Anchor.Value = not buttonStates.Anchor.Value
    ...
    elseif input.KeyCode == Enum.KeyCode.Space and buttonStates.Anchor.Value then vertControl = 1
    elseif input.KeyCode == Enum.KeyCode.LeftShift and buttonStates.Anchor.Value then vertControl = -1
    end
end)
...
local function createNovaGUI(character)
    ...
    -- FastSpeedSteal -> GUI1 visibility
...
-- Helper to find player's own MultiplierPart
...
-- ProximityPrompt teleport logic
