    ...
    for _, prompt in ipairs(workspace:GetDescendants()) do
        if prompt:IsA("ProximityPrompt") then
            prompt.Triggered:Connect(function(triggeringPlayer)
                -- ensure it's this player who triggered (client-side triggers pass player on client)
                ...
                -- show in-progress text on teleport button (visual only)
                local tb = buttonsMap["Teleport"]
                if tb then
                ...
                -- Temporarily enable Anchor and Noclip IF THEY ARE OFF — but IMPORTANT:
                -- we do NOT change the underlying BoolValues (buttonStates.Anchor/ Noclip).
                -- Instead we change the character parts directly and change GUI visuals temporarily.
                local anchorWasOn = buttonStates.Anchor.Value
                local noclipWasOn = buttonStates.Noclip.Value

                -- store references to parts to restore CanCollide
                local partsState = {}
                ...
                -- temporarily apply anchor/noclip visuals/effects
                if not anchorWasOn and root then
                    root.Anchored = true
                    -- update GUI visuals for Anchor button temporarily (without changing BoolValue)
                ...
                -- After teleport ends, restore parts and GUI visuals (but DO NOT toggle BoolValues)
                task.spawn(function()
                    while teleportInProgress do RunService.RenderStepped:Wait() end
                    task.wait(7) -- small delay

                    -- restore anchor physical state
                    if not anchorWasOn and root then
                        root.Anchored = false
                        -- restore Anchor GUI visuals according to BoolValue (still false)
                        local abtn = buttonsMap["Anchor"]
                        if abtn then
                    ...
                    -- restore teleport button appearance to match underlying BoolValue
                    local tb2 = buttonsMap["Teleport"]
                    if tb2 then
                        tb2.Text = buttonStates.Teleport.Value and "InstantSteal ON" or "InstantSteal OFF"
                        tb2.BackgroundColor3 = buttonStates.Teleport.Value and Color3.fromRGB(60,60,60) or Color3.fromRGB(150,150,150)
                    end
                end)
            end)
        end
